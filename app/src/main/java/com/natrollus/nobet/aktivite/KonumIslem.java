...
class KonumIslem extends AsyncTask<Void, Void, Void> implements GoogleApiClient.ConnectionCallbacks, GoogleApiClient.OnConnectionFailedListener, android.location.LocationListener {

	private GoogleApiClient googleApiClient;
	private Activity activity;
	private LocationRequest locationRequest;

	KonumIslem(Activity activity) {
	...
	public void onLocationChanged(Location location) {
		logla("degisti:"+location.toString());
	}

	@Override
	public void onStatusChanged(String s, int i, Bundle bundle) {
		logla("durum degisti s:"+s);
	}
	...
	public void onProviderDisabled(String s) {
		logla("provider not ok:"+s);
	}

	@Override
	public void onConnected(@Nullable Bundle bundle) {
		logla("konum islemden baglandi b:"+bundle);
		if (ActivityCompat.checkSelfPermission(activity, Manifest.permission.ACCESS_FINE_LOCATION) != PackageManager.PERMISSION_GRANTED && ActivityCompat.checkSelfPermission(activity, Manifest.permission.ACCESS_COARSE_LOCATION) != PackageManager.PERMISSION_GRANTED) {
